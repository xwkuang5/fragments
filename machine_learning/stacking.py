...
class StackingLearner:
    ...
    def prepare_stacking_data(self, X_train, X_test, Y_train, Y_test):
        ...
        for train_idx, test_idx in self._skf.split(X_train, Y_train):
            ...
            prediction_folds.append(np.stack([estimator.predict(X_test_fold) for estimator in self._list_of_estimators]))
        ...
        stack_train_X = self._feature_encoder.transform(stack_train_X.flatten()).reshape((-1, self._num_estimators))
        stack_test_X = self._feature_encoder.transform(stack_test_X.flatten()).reshape((-1, self._num_estimators))
    ...
    def train(self, X_train, Y_train, meta_learner):
        ...
        stack_train_X, stack_test_X, stack_train_Y, stack_test_Y = self.prepare_stacking_data(X_train, X_test, Y_train, Y_test)
    ...
    def predict(self, X_test):
        ...
        stack_test_X = np.transpose(np.stack([estimator.predict(X_test) for estimator in self._list_of_estimators]))
...
train_csv['Distance'] = train_csv.apply(lambda x: np.sqrt(x['XCoord']**2+x['YCoord']**2), axis=1)
test_csv['Distance'] = test_csv.apply(lambda x: np.sqrt(x['XCoord']**2+x['YCoord']**2), axis=1)
...
meta_learner_test_score = meta_learner.train(X_train, Y_train, DecisionTreeClassifier())

### Create meshgrid for visulization
xx, yy = np.meshgrid(np.arange(-1, 1, 0.01), np.arange(-1, 1, 0.01))
xxyy = np.c_[xx.ravel(), yy.ravel()]
xxyyzz = np.concatenate([xxyy, np.apply_along_axis(lambda x: np.sqrt(x[0]**2+x[1]**2), axis=1, arr=xxyy).reshape(-1, 1)], axis=1)

knn_predictions = meta_learner._feature_encoder.transform(best_knn_clf.predict(xxyyzz)).reshape(xx.shape)
svc_predictions = meta_learner._feature_encoder.transform(best_svc_clf.predict(xxyyzz)).reshape(xx.shape)
meta_predictions = meta_learner._feature_encoder.transform(meta_learner.predict(xxyyzz)).reshape(xx.shape)
...
axarr[0].scatter(X_train[:, 0], X_train[:, 1], c=training_data_points, cmap=plt.cm.Spectral)
axarr[0].set_title("KNN Decision Boundary (score: {:.2f})".format(best_knn_clf.score(X_test, Y_test)))

axarr[1].contourf(xx, yy, svc_predictions, cmap=plt.cm.Spectral)
axarr[1].scatter(X_train[:, 0], X_train[:, 1], c=training_data_points, cmap=plt.cm.Spectral)
axarr[1].set_title("SVC Decision Boundary (score: {:.2f})".format(best_svc_clf.score(X_test, Y_test)))

axarr[2].contourf(xx, yy, meta_predictions, cmap=plt.cm.Spectral)
axarr[2].scatter(X_train[:, 0], X_train[:, 1], c=training_data_points, cmap=plt.cm.Spectral)
axarr[2].set_title("Meta Learner Decision Boundary (score: {:.2f})".format(meta_learner_test_score))
