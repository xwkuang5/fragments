...
void AVLTreeNode ::print_node() {
  ...
  std :: cout << "key: " << key_ << ", height: " << height_ << ", parent: " << parent << ", left child: " << left << ", right child: " << right << std::endl;
}
...
void AVLTree ::rotate_right(node_ptr z) {
  ...
  if (y->right_ != NULL) {
    y->right_->parent_ = z;
    y->right_->is_left_ = true;
    y->right_->is_right_ = false;
  }
  y->right_ = z;
  z->parent_ = y;
  y->parent_ = parent;
...
void AVLTree ::rotate_left(node_ptr z) {
  ...
  if (y->left_ != NULL) {
    y->left_->parent_ = z;
    y->left_->is_left_ = false;
    y->left_->is_right_ = true;
  }
  y->left_ = z;
  z->parent_ = y;
  y->parent_ = parent;
