  ...
  while (i < pattern.size()) {
    ...
    if (pattern[i] == pattern[len]) {
      ...
      if (len == 0) {
        ...
        /*
         *  Suppose we are trying to compute the longest prefix suffix array
         * (lps) for the following pattern: \begin{equation*} a_0, a_1, \cdots,
         * a_{-1}, a^\star \end{equation*} Let's further suppose that
         * $\text{lps}[-2] = m$, i.e., $"a_0, \cdots, a_{m-1}"$ is the same as
         * $"a_{-m}, \cdots, a_{-1}"$ and $a_m != a^\star$. Now, because $a_m !=
         * a^\star$, we should decrease the length that we are trying to match,
         * in particular, according to the algorithm we set $\text{len} =
         * \text{lps}[m-1]$ \footnote{Note that we can not simply update len by
         * $\text{len} = \text{len} - 1$ because the resulting match position is
         * not the same. Consider the pattern "acaacc", using the update rule
         * just mentioned will result in incorrect value for the last
         * position.}. Note that we know the following: \begin{align*}
         *          a_0 & = a_{-m}\\
         *          a_1 & = a_{-(m-1)}\\
         *          & \cdots\\
         *          a_{m-1} & = a_{-1}
         ...
         *          a_{l-1} & = a_{-1}
         *  \end{align*}
         *  Because of the above, we can safely continue matching from
         * $\text{pat}[l] == a^\star$
         */
        len = lps[len - 1];
      }
...
void kmp_match(const std::string &content, const std::string &pattern) {
  ...
  while (i < content_size) {
    ...
    // content and pattern do not match, go to previous location where match
    // happens
    else {
      ...
      // if index for pattern is not 0, move index for pattern to previous
      // location
      else {
        j = lps[j - 1];
      }
