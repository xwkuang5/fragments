...
public:
    ...
    LinkedList(value_type value) {
        start_ = new node_type(value);
    }
    ...
    class iterator {
        public:
            ...
            // postfix
            self_type operator++() { self_type current = iterator(*this); this->_ptr = this->_ptr->next_; return current; }

            // postfix
            self_type& operator++(int dummy) { this->_ptr = this->_ptr->next_; return *this; }

            reference operator*() { return *this->_ptr; } // member access (./->) has precedence over dereference
