...
public:
    Graph(int m) : num_vertex(m) {
        ...
        for (int i = 0; i < m; i++) {
            // somehow the allocation of the second dimension does not follow the first dimension
    ...
    Graph(int m, int **adj_matrix) : num_vertex(m) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            std::copy(&adj_matrix[i][0], &adj_matrix[i][0] + num_vertex, &this->adj_matrix[i][0]);
        }
    }
    ...
    Graph(const Graph& g) : num_vertex(g.num_vertex) {
        ...
        for (int i = 0; i < num_vertex; i++) {
            ...
            std::copy(&g.adj_matrix[i][0], &g.adj_matrix[i][0] + num_vertex, &adj_matrix[i][0]);
        }
    }
